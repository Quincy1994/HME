...
// edge weight
int *mw_edge_source_id, *mw_edge_target_id;
...
void LoadVertex()
{
    ...
    for (int k = 0; k != num_edges; k++){
        fscanf(fin, "%s %s %lf", name_v1, name_v2, &weight);
        if (k % 10000 == 0){
            printf("Reading user word edges: %.3lf%%%c", k / (double)(num_edges + 1) * 100, 13);
            fflush(stdout);
        }
        vid_1 = SearchHashTable(name_v1);
        if (vid_1 == -1) vid_1 = AddVertex(name_v1);
    ...
    fclose(fin);
    printf("Number of user word vertices: %d         \n", num_vertices);
    ...
    for(int k = 0; k != num_edges; k++){
        ...
        if(k % 10000 == 0){
            printf("Reading user message edges: %.3lf%%%c", k / (double)(num_edges + 1) * 100, 13);
            fflush(stdout);
        }
        vid_1 = SearchHashTable(name_v1);
        if (vid_1 == -1) vid_1 = AddVertex(name_v1);
    ...
    fclose(fin);
    printf("Number of user message vertices: %d          \n", num_vertices);
}
...
void *TrainMWLINEThread(void *id)
{
	...
	while (1)
	{
		...
		if (count - last_count > 10000)
		{
			...
			last_count = count;
			printf("%cRho: %f  train MW Progress: %.3lf%%\n", 13, rho, (real)mw_current_sample_count / (real)(mw_total_samples + 1) * 100);
			fflush(stdout);
...
void *TrainUWLINEThread(void *id)
{
	...
	while (1)
	{
		...
		if (count - last_count > 10000)
		{
			...
			last_count = count;
			printf("%cRho: %f  train UW Progress: %.3lf%%\n", 13, rho, (real)uw_current_sample_count / (real)(uw_total_samples + 1) * 100);
			fflush(stdout);
//			rho = init_rho * (1 - current_sample_count / (real)(total_samples + 1));
//			if (rho < init_rho * 0.0001) rho = init_rho * 0.0001;
		}
...
void *TrainUMLINEThread(void *id)
{
	...
	while (1)
	{
		...
		if (count - last_count > 10000)
		{
			...
			last_count = count;
			printf("%cRho: %f  train UM Progress: %.3lf%%\n", 13, rho, (real)um_current_sample_count / (real)(um_total_samples + 1) * 100);
			fflush(stdout);
//			rho = init_rho * (1 - current_sample_count / (real)(total_samples + 1));
//			if (rho < init_rho * 0.0001) rho = init_rho * 0.0001;
		}
...
void TrainHUELINE() {
	...
	for(int i=0; i< iteration; i++){
	    ...
	    pthread_t *pt1 = (pthread_t *)malloc(num_threads * sizeof(pthread_t));
	    for (a = 0; a < num_threads; a++) pthread_create(&pt1[a], NULL, TrainMWLINEThread, (void *)a);
	    for (a = 0; a < num_threads; a++) pthread_join(pt1[a], NULL);
//	    free(pt);
	    printf("--------------- train um network --------------------\n");
	    pthread_t *pt2 = (pthread_t *)malloc(num_threads * sizeof(pthread_t));
	    for (a = 0; a < num_threads; a++) pthread_create(&pt2[a], NULL, TrainUMLINEThread, (void *)a);
	    for (a = 0; a < num_threads; a++) pthread_join(pt2[a], NULL);
//	    free(pt);
	    printf("--------------- train uw network --------------------\n");
	    pthread_t *pt3 = (pthread_t *)malloc(num_threads * sizeof(pthread_t));
	    for (a = 0; a < num_threads; a++) pthread_create(&pt3[a], NULL, TrainUWLINEThread, (void *)a);
	    for (a = 0; a < num_threads; a++) pthread_join(pt3[a], NULL);
	    printf("--------------------------------\n");
//	    free(pt);
